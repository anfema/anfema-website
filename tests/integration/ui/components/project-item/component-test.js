...
describe('Integration | Component | project item', function() {
	...
	it('renders', async function() {
		...
		assert.equal(
			await find('[data-test-project-item-client]').innerText,
			this.get('project').client
		);
		assert.equal(
			await find('[data-test-project-item-title]').innerText,
			this.get('project').title
		);
		assert.equal(
			await find('[data-test-project-item-color]').innerText,
			this.get('project').color
		);
	});
