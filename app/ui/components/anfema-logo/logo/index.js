...
export class AnfemaLogo {
    ...
    drawPhase(phase) {
        ...
        const step = direction === AnimationDirection.Forward
            ? this.currentStep
            : this.nextStep();
        const easedPhase = easeInOut(direction === AnimationDirection.Forward ? phase : 1.0 - phase, expoEaseIn, expoEaseOut);
        clearCanvas(this.context, this.options);
        context.save();
        context.translate((this.canvas.width - animation.boundingBox.w * this.scaleFactor) * 0.5, (this.canvas.height - animation.boundingBox.h * this.scaleFactor) * 0.5);
        animation.shapes.forEach(shape => { this.drawShape(shape, easedPhase, step); });
        context.restore();
    }
    ...
    mapDirection(toStep, direction) {
        switch (direction) {
            case StepDirection.Backward: return AnimationDirection.Backward;
            case StepDirection.Forward: return AnimationDirection.Forward;
    ...
    transitionToEnqueuedStateOr(state) {
        ...
        if (prevState !== this.currentState) {
            ...
            if (this.currentState === State.LoopingWaiting) {
                const delay = this.currentStep === 0
